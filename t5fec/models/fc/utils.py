...
@backoff.on_exception(backoff.expo, (Exception, TimeoutError), max_tries=300)
def completions_with_backoff(**kwargs):
    try:
        return client.completions.create(**kwargs, timeout=60)
...
class OpenAIModel:
    ...
    def generate(self, input_string, temperature = 0.0):
        if self.model_name in ['text-davinci-002', 'code-davinci-002', 'text-davinci-003']:
            ...
            while True: # retry if error occurs in opena
                try:
